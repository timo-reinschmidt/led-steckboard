...
public class ClientApp {
	...
	public static void showSquare(LedService service) {
		...
		do {
			...
			System.out.print("Bitte geben Sie einen gültigen Wert für Seitenlänge ein.: ");
			lengthSide = sc.nextInt();

			if (((topLeft + 1) % LedService.MAX_COLUMNS) - lengthSide >= 0
					&& (topLeft / LedService.MAX_ROWS) + 1 >= lengthSide && topLeft > 16 && lengthSide >= 2) {
				isCorrect = false;
			}
	...
	public static void showRectangle(LedService service) {
		...
		do {
			...
			if (topLeft >= bottomRight + 17 && topLeft < LedService.MAX_NUMBER_OF_LEDS
					&& topLeft % LedService.MAX_ROWS > bottomRight % LedService.MAX_ROWS && topLeft > 0
					&& bottomRight >= 0) {
				isCorrectTwo = false;
			}
		} while (isCorrectTwo);
	...
	public static void siebDesEratothenes(LedService service) {
		...
		// Schalte alle LEDs aus die ein vielfaches von i sind aber nicht die Zahl i
		// sind
